...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            println!("🚀 开始 Tauri 应用设置...");
            
            // 【统一优化】同步初始化关键存储组件 - 确保在前端访问前完成
            log::info!("🔧 开始同步初始化关键存储组件...");
            println!("🔧 开始同步初始化关键存储组件...");
            
            // 1. 优先初始化存储服务（同步，快速完成）
            if let Some(storage_state) = app.try_state::<storage_commands::StorageState>() {
                match storage_state.init(&app_handle) {
                    Ok(_) => {
                        log::info!("✅ 存储服务同步初始化成功");
                        println!("✅ 存储服务同步初始化成功");
                    },
                    Err(e) => {
                        log::error!("❌ 存储服务同步初始化失败: {}", e);
                        eprintln!("❌ 存储服务同步初始化失败: {}", e);
                        // 存储初始化失败是严重问题，但不应阻止应用启动
                        // 依赖自动重试机制来恢复
                    }
                }
            } else {
            ...
            // 2. 异步初始化其他非关键组件（不阻塞UI）
            let app_handle_clone = app_handle.clone();
            tauri::async_runtime::spawn(async move {
                log::info!("🔧 开始异步初始化非关键组件...");
                
                // 执行其他非关键的初始化任务
                match initialize_non_critical_components(&app_handle_clone).await {
                    Ok(_) => {
                        log::info!("✅ 非关键组件异步初始化完成");
                        println!("✅ 非关键组件异步初始化完成");
                    },
                    Err(e) => {
                        log::warn!("⚠️ 非关键组件初始化失败: {}", e);
                        eprintln!("⚠️ 非关键组件初始化失败: {}", e);
                        // 非关键组件失败不影响核心功能
                    }
                }
            });
